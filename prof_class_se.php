...
$total_temp=$class_details->findOne(['year' => 'SE','branch' => $branch] );
...
$temp=array();
	...
	if($temp0=='SE'){$index='0';  $sub=$temp_prof->year_sub0->sub0;}

	elseif($temp1=='SE'){$index='1'; $sub=$temp_prof->year_sub1->sub1;}

	
	elseif($temp2=='SE'){$index='2'; $sub=$temp_prof->year_sub2->sub2;}
 
	
	elseif($temp3=='SE'){$index='3'; $sub=$temp_prof->year_sub3->sub3;}
...
$total_lectures=$prof_class_att->count(['prof_name'=>$username,'class'=>'SE','subject'=>$sub]);
$att_count_se=0;
/*$temp=$prof_class_att->aggregate(['$group'=>['_id'=>['prof_name'=>$username,'class'=>'TE','subject'=>$sub], '$att_count_te'=>['$sum'=>'att_count']]]); 
*/
$temp1=$prof_class_att->find(['prof_name'=>$username,'class'=>'SE','subject'=>$sub]);

foreach ($temp1 as $key ) {
	$a= $key->att_count;
	$att_count_se = $att_count_se + $a ;
}

if($att_count_se){$result = (($att_count_se) / ($total_lectures * 10))*100;}
else {$result=0;}
